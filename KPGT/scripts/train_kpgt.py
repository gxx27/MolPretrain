...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(
        train_dataset, sampler=DistributedSampler(train_dataset), batch_size=batch_size1 // args.n_devices, 
    ...
    if 'mix' not in args.pretrain1_path and args.pretrain2_path is not None:
        batch_size2 = config['batch_size']
        # batch_size2 = 2048
        # args.n_steps = 200000
        train_dataset = MoleculeDataset(root_path=args.pretrain2_path)
        train_loader = DataLoader(
            train_dataset, sampler=DistributedSampler(train_dataset), batch_size=batch_size2 // args.n_devices, 
            num_workers=args.n_threads, worker_init_fn=seed_worker, drop_last=True, collate_fn=collator
        )
